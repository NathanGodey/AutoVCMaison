...
def make_spec(datasetDir = "training_set"):
    ...
    for dirName, dirs, files in os.walk(rootDir):
        targetDirName = targetDir + "/" + "/".join(dirName.split("\\")[1:])
        for subdir in dirs:
            if not os.path.exists(os.path.join(targetDirName, subdir)):
                os.makedirs(os.path.join(targetDirName, subdir))
        for fileName in files:
            ...
            # save spect    
            np.save(os.path.join(targetDirName, fileName[:-4]),
